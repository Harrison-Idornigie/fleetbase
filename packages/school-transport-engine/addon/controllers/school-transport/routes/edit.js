...
export default class SchoolTransportRoutesEditController extends Controller {
  ...
  get stepTitle() {
    ...
    return titles[this.currentStep] || 'Edit Route';
  }

  get stepDescription() {
    const descriptions = {
      1: 'Update the basic route information and settings',
      2: 'Modify the route stops and waypoints',
      3: 'Change vehicle and driver assignments',
      4: 'Adjust schedule and additional configurations'
    };
  ...
  get availableBackupRoutes() {
    return this.model.routes?.filter(r => r.id !== this.route?.id) || [];
  }
  ...
  initializeFormData() {
    if (this.route) {
      ...
      this.routeNumber = this.route.route_number || '';
      this.type = this.route.type || 'morning';
      this.status = this.route.status || 'active';
      this.distance = this.route.distance || '';
      this.estimatedDuration = this.route.estimated_duration || '';
      this.startTime = this.route.start_time || '';
      this.endTime = this.route.end_time || '';

      // Stops and waypoints
      this.stops = (this.route.stops || []).map(stop => ({ ...stop }));
      this.waypoints = (this.route.waypoints || []).map(waypoint => ({ ...waypoint }));

      // Vehicle assignment
      this.assignedVehicleId = this.route.assigned_vehicle_id || '';
      this.assignedDriverId = this.route.assigned_driver_id || '';

      // Schedule and recurrence
      this.isRecurring = this.route.is_recurring || false;
      this.recurrencePattern = this.route.recurrence_pattern || 'daily';
      this.recurrenceDays = this.route.recurrence_days || [];
      this.startDate = this.route.start_date || '';
      this.endDate = this.route.end_date || '';

      // Additional settings
      this.maxCapacity = this.route.max_capacity || '';
      this.specialInstructions = this.route.special_instructions || '';
      this.weatherDependent = this.route.weather_dependent || false;
      this.backupRouteId = this.route.backup_route_id || '';
    }
  }
  ...
  async saveRoute() {
    ...
    this.isLoading = true;
    try {
      // Update route model with form data
      this.route.name = this.name;
      this.route.description = this.description;
      this.route.route_number = this.routeNumber || null;
      this.route.type = this.type;
      this.route.status = this.status;
      this.route.distance = parseFloat(this.distance) || null;
      this.route.estimated_duration = this.estimatedDuration;
      this.route.start_time = this.startTime;
      this.route.end_time = this.endTime;
      this.route.stops = this.stops;
      this.route.waypoints = this.waypoints;
      this.route.assigned_vehicle_id = this.assignedVehicleId;
      this.route.assigned_driver_id = this.assignedDriverId;
      this.route.is_recurring = this.isRecurring;
      this.route.recurrence_pattern = this.recurrencePattern;
      this.route.recurrence_days = this.recurrenceDays;
      this.route.start_date = this.startDate;
      this.route.end_date = this.endDate;
      this.route.max_capacity = parseInt(this.maxCapacity) || null;
      this.route.special_instructions = this.specialInstructions;
      this.route.weather_dependent = this.weatherDependent;
      this.route.backup_route_id = this.backupRouteId;

      await this.route.save();

      this.notifications.success('Route updated successfully');
      this.router.transitionTo('school-transport.routes.view', this.route.id);
    } catch (error) {
      console.error('Error saving route:', error);
      this.notifications.error('Failed to update route');
    } finally {
      this.isLoading = false;
    }
  }
  ...
  @action
  async createBackupRoute() {
    this.isLoading = true;
    try {
      const backupRoute = this.store.createRecord('school-transport/route', {
        name: `${this.route.name} (Backup)`,
        description: `Backup route for ${this.route.name}`,
      ...
      await backupRoute.save();
      this.backupRouteId = backupRoute.id;
      this.notifications.success('Backup route created successfully');
    } catch (error) {
      console.error('Error creating backup route:', error);
      this.notifications.error('Failed to create backup route');
    } finally {
      this.isLoading = false;
    }
  }
